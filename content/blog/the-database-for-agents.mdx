...
- The full migration from Next.js happened over a weekend: 27 packages, 80+ MCP tools, zero AWS
</Callout>
...
| One monolith, 520 routes | 27 packages, each does one thing |
...
│ sql → D1 database   │ sql → sql.js / WASM  │ sql → better-sqlite3 │
│ blob→ R2 bucket     │ blob→ OPFS           │ blob→ fs (optional)  │
└─────────────────────┴──────────────────────┴──────────────────────┘
```

The D1 adapter wraps Cloudflare's D1 binding. The IDB adapter wraps IndexedDB with sql.js for SQL support. The memory adapter uses in-memory Maps — perfect for tests. Your block code never knows which one it's talking to.
...
**Deployed:**
- 27 packages, each does one thing
- D1 database with 17 tables in spike-land-mcp alone (Drizzle-managed)
- 80+ MCP tools
- 5 Cloudflare Workers (spike-edge, spike-land-mcp, mcp-auth, spike-land-backend, transpile)
- 0 servers to manage
...
Twenty-seven packages. Eighty-plus MCP tools. Five Cloudflare Workers. Zero servers. One `defineBlock()` call that packages schema, business logic, and MCP tools into a composable unit that runs anywhere.
