...
function betaPdf(x: number, alpha: number, beta: number): number {
  if (x === 0 || x === 1) return 0;
  // Approximation without Gamma function for visualization purposes
  // Scaled to fit nicely in our view
  const maxVal =
    Math.pow((alpha - 1) / (alpha + beta - 2), alpha - 1) *
    Math.pow((beta - 1) / (alpha + beta - 2), beta - 1);
  const val = Math.pow(x, alpha - 1) * Math.pow(1 - x, beta - 1);
  return val / (maxVal || 1); // Normalize to 1 at peak
}
...
export const BayesianConfidenceCore: FC<BayesianConfidenceCoreProps> = ({
  ...
  // Prior is alpha=2, beta=2 (mildly confident it works 50% of time)
