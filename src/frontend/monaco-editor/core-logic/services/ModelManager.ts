import { Code } from "../../concurrency/code-session";
import type { IExtraModelsResult, IModelManager } from "../../ui/@/lib/interfaces";
...
export class ModelManager implements IModelManager {
  ...
  async getCurrentCodeWithExtraModels(): Promise<string> {
    ...
    const currentSession = await currentModel.getSession();
    const extraModels = await this.fetchAndCreateExtraModels(currentSession.code, location.origin);
    ...
    const currentCodeSection = this.formatCodeAsSection(this.currentCodeSpace, currentSession.code);
