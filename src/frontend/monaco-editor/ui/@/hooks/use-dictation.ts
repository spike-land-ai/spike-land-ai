...
export function useDictation(defaultValue = "", options: UseDictationOptions = {}) {
  ...
  const startRecording = useCallback(async () => {
    ...
    const RecordRTCC = (await import("../../../media-capture/record-rtc")).default;
  ...
  const processAudio = async (audioBlob: Blob) => {
    ...
    if (sendError) {
      ...
      setError(
        `An error occurred: ${sendError instanceof Error ? sendError.message : String(sendError)}`,
      );
      return;
    }
    ...
    if (textError) {
      ...
      setError(
        `An error occurred while reading response: ${textError instanceof Error ? textError.message : String(textError)}`,
      );
      return;
    }
    ...
    setMessage(
      (prevMessage) => prevMessage + " " + text.replace(/^"|"$/g, "").replace(/\\n/g, "\n").trim(),
    );
  };
