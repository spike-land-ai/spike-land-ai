...
import { EditorNode } from "../../animation-ui/ErrorReminder";
...
export const Editor: React.FC<EditorProps> = ({ codeSpace, cSess, replaceIframe }) => {
  ...
  useEffect(() => {
    ...
    const sortSession = async () => {
      const { data: sortedSession, error } = await tryCatch(cSess.getSession());
      if (error) {
    ...
    async (newCode: string) => {
      ...
      const processChange = async (code: string) => {
        ...
        if (newHash !== lastHash) {
          ...
          if (newCode !== code) {
            const { data: finalCode, error: saveError } = await tryCatch(cSess.setCode(code, true));
            if (saveError) {
  ...
  return (
    <div className="relative flex h-screen w-full max-w-[800px] overflow-hidden">
