...
export const useErrorHandling = (engine: string) => {
  const [errorType, setErrorType] = useState<"typescript" | "transpile" | "render" | null>(null);

  const throttledTypeCheck = useCallback(async () => {
    if (engine === "monaco") {
      const { editor, typescript } = await import("@/workers/monaco-editor.worker");
