...
export function registerSkillStoreTools(
      ...
      .tool(
        ...
        "List published skills from the skill store with optional category and search filters.",
        {
          ...
          search: z
            ...
            .max(200)
            .optional()
            .describe("Search skills by name or description."),
          limit: z
            .number()
            .int()
            .min(1)
  ...
  registry.registerBuilt(
    t
      .tool(
        "skill_store_get",
        "Get detailed information about a specific skill by slug or ID.",
        {
  ...
  registry.registerBuilt(
    t
      .tool(
        "skill_store_admin_list",
        "List all skills including drafts and archived (admin).",
        {
          status: z
            .enum(SKILL_STATUSES)
            .optional()
    ...
    t
      .tool(
        ...
        "Create a new skill in the store (admin). Delegates to spike.land API.",
        {
          ...
          status: z.enum(SKILL_STATUSES).optional().default("DRAFT").describe("Publication status."),
          version: z.string().max(20).optional().default("1.0.0").describe("Semantic version string."),
          ...
          tags: z.array(z.string().max(50)).max(20).optional().default([]).describe("Searchable tags (up to 20)."),
          sortOrder: z.number().int().min(0).optional().default(0).describe("Display sort order (lower = earlier)."),
          isActive: z.boolean().optional().default(true).describe("Whether the skill is active."),
          isFeatured: z.boolean().optional().default(false).describe("Whether the skill is featured."),
        },
      )
      .meta({ category: "skill-store", tier: "free" })
      .handler(async ({ input }) => {
    ...
    t
      .tool(
        ...
        "Update fields of an existing skill (admin). Delegates to spike.land API.",
        {
          ...
          name: z.string().min(1).max(100).regex(/^[a-z0-9-]+$/).optional().describe("URL-friendly skill name."),
          slug: z.string().min(1).max(100).regex(/^[a-z0-9-]+$/).optional().describe("URL-friendly slug."),
          displayName: z.string().min(1).max(200).optional().describe("Human-readable display name."),
          description: z.string().min(1).max(500).optional().describe("Short skill description."),
          longDescription: z.string().max(10000).optional().describe("Extended markdown description."),
          ...
          tags: z.array(z.string().max(50)).max(20).optional().describe("Searchable tags (up to 20)."),
