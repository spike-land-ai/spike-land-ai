...
export const SkillInputSchema = z.object({
  ...
  proficiency: z.number().int().min(1).max(5).describe(
    "Self-assessed proficiency 1-5",
  ),
});

export const AssessSkillsSchema = z.object({
  skills: z.array(SkillInputSchema).min(1).max(50).describe(
...
export const CompareSkillsSchema = z.object({
  ...
  occupationUri: z.string().min(1).describe(
    "ESCO occupation URI to compare against",
  ),
});

export const GetSalarySchema = z.object({
  occupationTitle: z.string().min(1).describe(
    "Occupation title for salary lookup",
  ),
