...
export function registerJulesTools(
  ...
  registry.register({
    ...
    handler: async (
      { status, page_size }: { status?: string; page_size: number; },
  ...
  registry.register({
    ...
    handler: async (
      { title, task, source_repo, starting_branch }: {
        title: string;
        task: string;
        source_repo?: string;
  ...
  registry.register({
    ...
    handler: async (
      { session_id, message }: { session_id: string; message: string; },
    ): Promise<CallToolResult> => {
      validateSessionId(session_id);
      const name = session_id.startsWith("sessions/")
        ? session_id
