...
export function registerMcpObservabilityTools(
  ...
  registry.register({
    name: "admin_tool_metrics",
    description:
      "Per-tool MCP metrics: call count, p50/p95/p99 latency, error rate, and token usage.\n"
      + "Query the ToolInvocation table for aggregated metrics over a configurable time window.",
    category: "mcp-observability",
    tier: "workspace",
    inputSchema: ToolMetricsSchema.shape,
    handler: async ({
      period_hours = 24,
      tool_name,
      limit = 20,
    }: z.infer<typeof ToolMetricsSchema>): Promise<CallToolResult> =>
      safeToolCall("admin_tool_metrics", async () => {
  ...
  registry.register({
    ...
    description: "MCP system health: database/Redis latency, total invocations, error rate, "
      + "active tools, and optionally a per-tool error breakdown.",
    category: "mcp-observability",
    tier: "workspace",
    inputSchema: SystemHealthSchema.shape,
    handler: async ({
      include_tool_breakdown = false,
    }: z.infer<typeof SystemHealthSchema>): Promise<CallToolResult> =>
      safeToolCall("admin_system_health", async () => {
        await requireAdminRole(userId);
        const prisma = (await import("@/lib/prisma")).default;
        ...
        text += `**Infrastructure:**\n`;
        text += `- Database: ${dbHealth.status}${
          dbHealth.latencyMs >= 0 ? ` (${dbHealth.latencyMs}ms)` : ""
        }\n`;
        text += `- Redis: ${redisHealth.status}${
          redisHealth.latencyMs >= 0 ? ` (${redisHealth.latencyMs}ms)` : ""
        }\n\n`;
  ...
  registry.register({
    ...
    description: "Per-user MCP analytics and cost attribution: calls, tokens, errors per user, "
      + "and daily token breakdown per tool per user.",
    category: "mcp-observability",
    tier: "workspace",
    inputSchema: UserAnalyticsSchema.shape,
    handler: async ({
      period_hours = 24,
      user_id,
      limit = 20,
    }: z.infer<typeof UserAnalyticsSchema>): Promise<CallToolResult> =>
      safeToolCall("admin_user_analytics", async () => {
        await requireAdminRole(userId);
        const prisma = (await import("@/lib/prisma")).default;
        const since = new Date(Date.now() - period_hours * 60 * 60 * 1000);
