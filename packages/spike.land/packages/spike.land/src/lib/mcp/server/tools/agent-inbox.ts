...
const SiteChatPollSchema = z.object({
  since: z
    .string()
    .optional()
    .describe("ISO timestamp. Only return messages created after this time."),
...
const SiteChatRespondSchema = z.object({
  message_id: z
    .string()
    .min(1)
...
export function registerAgentInboxTools(
  ...
  registry.register({
    name: "agent_inbox_poll",
    description: "Poll for new unread user messages across all your apps. "
    ...
    tier: "free",
    inputSchema: PollSchema.shape,
    handler: async (
      { since, app_id }: z.infer<typeof PollSchema>,
    ): Promise<CallToolResult> =>
      safeToolCall("agent_inbox_poll", async () => {
        ...
        for (const app of appMap.values()) {
          ...
          text += `- **Latest:** ${app.latestMessage.content}${
            app.latestMessage.content.length >= 200 ? "..." : ""
          }\n`;
  ...
  registry.register({
    ...
    description: "Get full messages for an app. Returns chronological messages with full content, "
      + "attachments, and code versions.",
    category: "agent-inbox",
    tier: "free",
    inputSchema: ReadSchema.shape,
    handler: async (
      { app_id, limit, unread_only, since }: z.infer<typeof ReadSchema>,
    ): Promise<CallToolResult> =>
      safeToolCall("agent_inbox_read", async () => {
        const prisma = (await import("@/lib/prisma")).default;
        ...
        let text = `**Messages for "${
          app.name || app.codespaceId || app_id
        }" (${messages.length}):**\n\n`;
        for (const msg of messages) {
          ...
          const readStatus = msg.role === "USER"
            ...
            : "";
          text += `**[${role}]${readStatus}** (${
            new Date(msg.createdAt).toISOString()
          }) ID: ${msg.id}\n`;
          text += `${msg.content}\n`;
          if (msg.attachments && msg.attachments.length > 0) {
            text += `Attachments: ${
              msg.attachments.map(a => a.image?.originalUrl || a.imageId)
                .join(", ")
            }\n`;
          }
          if (msg.codeVersion) {
  ...
  registry.register({
    name: "agent_inbox_respond",
    description: "Send an agent response to an app chat. Creates an AGENT message, "
      + "marks processed messages as read, and broadcasts to the user's browser via SSE.",
    category: "agent-inbox",
    tier: "free",
    inputSchema: RespondSchema.shape,
    handler: async (
      { app_id, content, code_updated, processed_message_ids }: z.infer<
        typeof RespondSchema
      >,
    ): Promise<CallToolResult> =>
      safeToolCall("agent_inbox_respond", async () => {
        const prisma = (await import("@/lib/prisma")).default;
  ...
  registry.register({
    name: "agent_inbox_site_chat_poll",
    description: "Poll for new site chat messages that need AI responses. "
      + "Returns questions where answer is null (pending).",
    category: "agent-inbox",
    tier: "free",
    inputSchema: SiteChatPollSchema.shape,
    handler: async (
      { since }: z.infer<typeof SiteChatPollSchema>,
    ): Promise<CallToolResult> =>
      safeToolCall("agent_inbox_site_chat_poll", async () => {
        const prisma = (await import("@/lib/prisma")).default;
        const { setMcpAgentActive } = await import("@/lib/upstash/client");
  ...
  registry.register({
    name: "agent_inbox_site_chat_respond",
    description: "Respond to a site chat message. Updates the BazdmegChatMessage with the answer.",
    category: "agent-inbox",
    tier: "free",
    inputSchema: SiteChatRespondSchema.shape,
    handler: async (
      { message_id, answer, model }: z.infer<typeof SiteChatRespondSchema>,
    ): Promise<CallToolResult> =>
      safeToolCall("agent_inbox_site_chat_respond", async () => {
        const prisma = (await import("@/lib/prisma")).default;
        ...
        return textResult(
          `**Site chat response saved.**\n\n`
            ...
            + `**Question:** ${message.question.slice(0, 100)}${
