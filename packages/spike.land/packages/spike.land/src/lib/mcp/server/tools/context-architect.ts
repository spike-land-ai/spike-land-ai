...
export function registerContextArchitectTools(
  ...
  registry.register({
    ...
    description: "Index a GitHub repository file tree for later context packing. "
    ...
    handler: async (
      { repo_url, branch }: { repo_url: string; branch: string; },
  ...
  registry.register({
    ...
    handler: async (
      { repo_url, task_description, max_files }: {
        repo_url: string;
  ...
  registry.register({
    ...
    description: "Get the import/dependency graph for a specific file in an indexed repository. "
    ...
    handler: async (
      { repo_url, file_path }: { repo_url: string; file_path: string; },
    ): Promise<CallToolResult> =>
