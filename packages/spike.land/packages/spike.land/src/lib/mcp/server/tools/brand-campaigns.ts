...
export function registerBrandCampaignTools(
  ...
  registry.register({
    ...
    description:
      "Create a marketing campaign. Stores campaign details including platform, budget, and start date.",
    category: "brand-campaigns",
    tier: "free",
    alwaysEnabled: true,
    inputSchema: CreateCampaignSchema.shape,
    handler: async (
      { name, description, platform, start_date, budget }: z.infer<
        typeof CreateCampaignSchema
      >,
    ): Promise<CallToolResult> =>
      safeToolCall("brand_create_campaign", async () => {
        ...
        return textResult(
          `**Campaign Created**\n\n`
            ...
            + `**Budget:** ${
              campaign.budget !== null ? `$${campaign.budget.toFixed(2)}` : "(not set)"
            }\n`
  ...
  registry.register({
    ...
    handler: async (
      { status, platform }: z.infer<typeof ListCampaignsSchema>,
    ): Promise<CallToolResult> =>
      safeToolCall("brand_list_campaigns", async () => {
  ...
  registry.register({
    ...
    description:
      "Get performance metrics for a campaign: impressions, clicks, engagement rate, reach, and cost per engagement.",
    category: "brand-campaigns",
    tier: "free",
    alwaysEnabled: true,
    inputSchema: GetCampaignStatsSchema.shape,
    handler: async (
      { campaign_id }: z.infer<typeof GetCampaignStatsSchema>,
    ): Promise<CallToolResult> =>
      safeToolCall("brand_get_campaign_stats", async () => {
        const campaigns = await loadCampaigns(userId);
  ...
  registry.register({
    ...
    description:
      "Generate A/B copy variants for a campaign with headlines and CTAs. Supports 2-5 variants across multiple tones.",
    category: "brand-campaigns",
    tier: "free",
    alwaysEnabled: true,
    inputSchema: GenerateVariantsSchema.shape,
    handler: async (
      { campaign_id, variant_count, tone }: z.infer<
        typeof GenerateVariantsSchema
      >,
    ): Promise<CallToolResult> =>
      safeToolCall("brand_generate_variants", async () => {
        const campaigns = await loadCampaigns(userId);
        const campaign = campaigns.find(c => c.id === campaign_id);
