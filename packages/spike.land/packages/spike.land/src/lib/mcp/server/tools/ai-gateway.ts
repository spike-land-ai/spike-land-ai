...
const ManageProviderSchema = z.object({
  action: z
...
export function registerAiGatewayTools(
  ...
  registry.register({
    ...
    description:
      "List configured AI providers with availability status, default provider, and capabilities.",
    category: "ai-gateway",
    tier: "free",
    inputSchema: ListProvidersSchema.shape,
    handler: async (
      _input: z.infer<typeof ListProvidersSchema>,
    ): Promise<CallToolResult> =>
      safeToolCall("ai_list_providers", async () => {
  ...
  registry.register({
    name: "ai_list_models",
    description: "List available AI models, optionally filtered by provider and/or capability.",
    category: "ai-gateway",
    tier: "free",
    ...
    handler: async ({
      provider = "all",
      capability = "all",
    }: z.infer<typeof ListModelsSchema>): Promise<CallToolResult> =>
      safeToolCall("ai_list_models", async () => {
  ...
  registry.register({
    name: "ai_chat",
    description:
      "Send a message to any configured AI provider (Anthropic or Google) with automatic provider/model detection.",
    category: "ai-gateway",
    tier: "workspace",
    inputSchema: AiChatSchema.shape,
    handler: async ({
      message,
      provider,
      model,
      ...
      max_tokens,
      temperature,
    }: z.infer<typeof AiChatSchema>): Promise<CallToolResult> =>
      safeToolCall("ai_chat", async () => {
  ...
  registry.register({
    name: "ai_manage_provider",
    description: "Manage AI provider credentials (upsert, delete, set default). Admin-only.",
    category: "ai-gateway",
    tier: "workspace",
    inputSchema: ManageProviderSchema.shape,
    handler: async ({
      action,
      provider_id,
      token,
      config,
    }: z.infer<typeof ManageProviderSchema>): Promise<CallToolResult> =>
      safeToolCall("ai_manage_provider", async () => {
  ...
  registry.register({
    name: "ai_get_usage",
    description: "Get AI usage statistics: total invocations, token counts, breakdown by provider.",
    category: "ai-gateway",
    tier: "free",
    inputSchema: GetUsageSchema.shape,
    handler: async ({
      days = 7,
      provider,
    }: z.infer<typeof GetUsageSchema>): Promise<CallToolResult> =>
      safeToolCall("ai_get_usage", async () => {
        const prisma = (await import("@/lib/prisma")).default;
