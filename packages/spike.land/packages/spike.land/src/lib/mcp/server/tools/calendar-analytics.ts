...
const SuggestContentSchema = z.object({
...
export function registerCalendarAnalyticsTools(
  ...
  registry.register({
    ...
    description:
      "Get posting analytics for a period and optional platform. Returns total posts, engagement rate, reach, impressions, and growth trend.",
    ...
    handler: async (
      { period, platform }: z.infer<typeof GetAnalyticsSchema>,
    ): Promise<CallToolResult> =>
      safeToolCall("calendar_get_analytics", async () => {
  ...
  registry.register({
    ...
    description:
      "Generate AI-powered content suggestions for a given topic and platform. Returns titles, descriptions, optimal posting times, and hashtags.",
    category: "calendar-analytics",
    tier: "free",
    alwaysEnabled: true,
    annotations: { readOnlyHint: true },
    inputSchema: SuggestContentSchema.shape,
    handler: async (
      { topic, platform, count }: z.infer<typeof SuggestContentSchema>,
    ): Promise<CallToolResult> =>
      safeToolCall("calendar_suggest_content", async () => {
        if (!topic.trim()) {
          return textResult(
  ...
  registry.register({
    ...
    description:
      "Schedule multiple social media posts at once. All scheduled_at values must be in the future. Returns post IDs and confirmation for each entry.",
    category: "calendar-analytics",
    tier: "free",
    alwaysEnabled: true,
    inputSchema: BulkScheduleSchema.shape,
    handler: async (
      { posts }: z.infer<typeof BulkScheduleSchema>,
    ): Promise<CallToolResult> =>
      safeToolCall("calendar_bulk_schedule", async () => {
        const prisma = (await import("@/lib/prisma")).default;
  ...
  registry.register({
    ...
    description:
      "Get detailed performance metrics for a specific post: likes, shares, comments, impressions, click-through rate, and peak engagement time.",
    category: "calendar-analytics",
    tier: "free",
    alwaysEnabled: true,
    annotations: { readOnlyHint: true },
    ...
    handler: async (
      { post_id }: z.infer<typeof GetPerformanceSchema>,
    ): Promise<CallToolResult> =>
      safeToolCall("calendar_get_performance", async () => {
        const prisma = (await import("@/lib/prisma")).default;
