...
export function registerCodeSpaceTools(
  ...
  registry.register({
    ...
    handler: async (
      { codespace_id, code, run }: z.infer<typeof UpdateCodeSchema>,
  ...
  registry.register({
    ...
    handler: async (
      { codespace_id }: z.infer<typeof CodeSpaceIdSchema>,
    ): Promise<CallToolResult> => {
      validateCodeSpaceId(codespace_id);
      try {
  ...
  registry.register({
    ...
    handler: async (
      { codespace_id, app_id, app_name, app_description }: z.infer<
        typeof LinkAppSchema
      >,
    ): Promise<CallToolResult> => {
      if (app_id) {
  ...
  registry.register({
    ...
    handler: async (
      { codespace_id, test_path }: { codespace_id: string; test_path?: string; },
    ): Promise<CallToolResult> => {
      validateCodeSpaceId(codespace_id);
      ...
      if (testFiles.length === 0) {
        return {
          content: [{
            type: "text",
            text: `No test files found in codespace "${codespace_id}".${
  ...
  registry.register({
    ...
    handler: async (
      { codespace_id, from_tests, from_version }: {
        codespace_id: string;
        from_tests?: boolean;
        from_version?: number;
      },
