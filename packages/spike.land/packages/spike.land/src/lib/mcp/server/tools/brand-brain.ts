...
export function registerBrandBrainTools(
  ...
  registry.register({
    ...
    description:
      "Score content against brand vocabulary and guardrails. Returns a score (0-100) with violations.",
    category: "brand-brain",
    tier: "free",
    inputSchema: ScoreContentSchema.shape,
    annotations: { readOnlyHint: true },
    handler: async (
      args: z.infer<typeof ScoreContentSchema>,
  ...
  registry.register({
    ...
    handler: async (
      args: z.infer<typeof RewriteContentSchema>,
    ): Promise<CallToolResult> =>
  ...
  registry.register({
    ...
    description:
      "Check content against active policy rules. Returns PASSED, PASSED_WITH_WARNINGS, or FAILED.",
    category: "brand-brain",
    tier: "free",
    inputSchema: CheckPolicySchema.shape,
    annotations: { readOnlyHint: true },
    handler: async (
      args: z.infer<typeof CheckPolicySchema>,
    ): Promise<CallToolResult> =>
  ...
  registry.register({
    ...
    annotations: { readOnlyHint: true },
    handler: async (
      args: z.infer<typeof ListViolationsSchema>,
    ): Promise<CallToolResult> =>
  ...
  registry.register({
    ...
    annotations: { readOnlyHint: true },
    handler: async (
      args: z.infer<typeof GetGuardrailsSchema>,
    ): Promise<CallToolResult> =>
  ...
  registry.register({
    ...
    description:
      "Analyze brand voice characteristics: tone, personality, vocabulary, and guardrails for a workspace.",
    category: "brand-brain",
    tier: "free",
    inputSchema: AnalyzeBrandVoiceSchema.shape,
    annotations: { readOnlyHint: true },
    handler: async (
      args: z.infer<typeof AnalyzeBrandVoiceSchema>,
    ): Promise<CallToolResult> =>
      safeToolCall("analyze_brand_voice", async () => {
        const prisma = (await import("@/lib/prisma")).default;
        ...
        text += `**Tone:** ${profile.toneDescriptors ?? "(none)"}\n\n`;
        text += `**Vocabulary**\n`;
        text += `- Preferred terms (${preferred.length}): ${
          preferred.slice(0, 10).map(v => v.term).join(", ") || "(none)"
        }\n`;
        text += `- Banned terms (${banned.length}): ${
          banned.slice(0, 10).map(v => v.term).join(", ") || "(none)"
        }\n\n`;
