 ...
 */
export const GridThumbnail = forwardRef<HTMLDivElement, GridThumbnailProps>(function GridThumbnail(
  ...
  const displayUrl =
    isSelected && image.enhancedUrl && !imageError ? image.enhancedUrl : image.originalUrl;
  ...
  const aspectRatioPadding = image.width && image.height ? (image.height / image.width) * 100 : 100; // Default to square if dimensions not available
  ...
  return (
    <div
      ...
      className={cn(
        ...
        isSelected && ["ring-2 ring-primary", "shadow-glow-cyan", "animate-pulse-cyan"],
        className,
      )}
