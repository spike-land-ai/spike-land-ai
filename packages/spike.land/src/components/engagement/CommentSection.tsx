 ...
 */
export function CommentSection({ contentType, contentSlug }: CommentSectionProps) {
  ...
  // Load comments
  useEffect(() => {
    fetch(`/api/comments?contentType=${contentType}&slug=${encodeURIComponent(contentSlug)}`)
      .then((r) => r.json())
      .then((data: { comments: CommentData[]; total: number }) => {
    ...
    <section className="mt-12 pt-8 border-t border-border/30">
      ...
      {loading ? (
        <div className="flex flex-col gap-3">
          {[1, 2, 3].map((i) => (
            <div key={i} className="animate-pulse rounded-xl bg-white/[0.03] h-20" />
          ))}
        </div>
      ) : comments.length === 0 ? (
