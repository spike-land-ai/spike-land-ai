...
export function McpCategoriesPageClient() {
  ...
  const filteredData = useMemo(() => {
    ...
    return MCP_SUPER_CATEGORIES.map((sup) => {
      ...
      const filteredSubs = sup.subcategories
        .map((sub) => {
          ...
          const filteredCats = sub.categories
            ...
            .filter(
              (item): item is { category: McpCategory; tools: McpToolDef[] } => item !== null,
            );
          ...
          return {
            ...
            _filteredTools: new Map(filteredCats.map((fc) => [fc.category.id, fc.tools])),
      ...
      (item): item is McpSuperCategory & { subcategories: FilteredSubcategory[] } => item !== null,
    );
  }, [searchQuery]);
  ...
  const getToolsForCategory = useCallback(
    (catId: string, sub: FilteredSubcategory): McpToolDef[] => {
  ...
  return (
    <main className="min-h-screen container mx-auto px-4 py-8">
      ...
      <div className="mb-8 space-y-2">
        <h1 className="text-3xl font-bold tracking-tight sm:text-4xl">Tool Categories</h1>
        <p className="text-muted-foreground text-lg">{SUPER_CATEGORY_COUNT} categories</p>
      </div>
            ...
            return (
              <section key={superCat.id}>
                <div className="flex items-center gap-3 mb-4 pb-2 border-b border-border/50">
                  ...
                  <h2 className="text-2xl font-bold tracking-tight">{superCat.name}</h2>
                ...
                >
                  {superCat.subcategories.map((sub) => {
                    const SubIcon = ICON_MAP[sub.icon] as
                      ...
                      | undefined;
                    return (
                      <AccordionItem key={sub.id} value={`${superCat.id}-${sub.id}`}>
...
function ToolItem({
  ...
  onExecute: (tool: McpToolDef, params: Record<string, unknown>) => Promise<void>;
}) {
  return (
    <div>
      <button
