...
function PollAnalyticsDashboardInline({ slug }: PollAnalyticsDashboardProps) {
  const [data, setData] = useState<Record<string, { votes_yes: number; votes_no: number }> | null>(
    null,
  );
  ...
  if (!data || Object.keys(data).length === 0) {
    return <p className="text-sm text-muted-foreground mt-4">Be the first to vote!</p>;
  }

  return (
    <div className="mt-6 space-y-3">
      <h4 className="text-sm font-semibold text-foreground">Results by persona</h4>
      {Object.entries(data).map(([persona, counts]) => {
        const total = counts.votes_yes + counts.votes_no;
        const yesPct = total > 0 ? Math.round((counts.votes_yes / total) * 100) : 0;
      ...
      {voted === null ? (
        ...
        <div>
          <p className="text-sm text-muted-foreground">
            You voted <span className="font-semibold">{voted === "yes" ? "Yes" : "No"}</span>.
            Thanks for sharing!
          </p>
