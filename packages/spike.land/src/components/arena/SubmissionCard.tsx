...
export function SubmissionCard({
  ...
  const failedWithNoWork =
    status === "FAILED" && iterations === 0 && inputTokens + outputTokens === 0;
  const firstError = failedWithNoWork && errors?.length ? errors[0]!.error : null;
  return (
    <div className="bg-zinc-900/50 border border-zinc-800 rounded-lg p-4">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          <span className="text-sm text-zinc-200">{userName || "Anonymous"}</span>
      ...
      <div className="flex items-center gap-4 text-xs text-zinc-500">
        ...
        {totalDurationMs && <span>{(totalDurationMs / 1000).toFixed(1)}s</span>}
        <span>
