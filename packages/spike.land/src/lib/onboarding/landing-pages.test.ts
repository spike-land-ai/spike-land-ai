...
describe("landing pages data", () => {
  ...
  it("has a landing page for every persona slug", () => {
    for (const persona of PERSONAS) {
      ...
      expect(landing, `Missing landing page for persona: ${persona.slug}`).toBeDefined();
    }
  });
  ...
  it("every landing page has a non-empty headline", () => {
    for (const lp of LANDING_PAGES) {
      expect(lp.headline.length, `Empty headline for ${lp.slug}`).toBeGreaterThan(0);
    }
  });

  it("every landing page has a non-empty subheadline", () => {
    for (const lp of LANDING_PAGES) {
      expect(lp.subheadline.length, `Empty subheadline for ${lp.slug}`).toBeGreaterThan(0);
    }
  });

  it("every landing page has exactly 3 pain points", () => {
    for (const lp of LANDING_PAGES) {
      expect(lp.painPoints, `Wrong pain point count for ${lp.slug}`).toHaveLength(3);
    }
  });
  ...
  it("every pain point has a non-empty title and description", () => {
    for (const lp of LANDING_PAGES) {
      for (const pp of lp.painPoints) {
        expect(pp.title.length, `Empty pain point title in ${lp.slug}`).toBeGreaterThan(0);
        expect(pp.description.length, `Empty pain point description in ${lp.slug}`).toBeGreaterThan(
          0,
        );
      }
    }
  ...
  it("every landing page has 3-4 features", () => {
    for (const lp of LANDING_PAGES) {
      expect(lp.features.length, `Wrong feature count for ${lp.slug}`).toBeGreaterThanOrEqual(3);
      expect(lp.features.length, `Too many features for ${lp.slug}`).toBeLessThanOrEqual(4);
    }
  });
  ...
  it("every feature has non-empty title, description, and appSlug", () => {
    for (const lp of LANDING_PAGES) {
      for (const feat of lp.features) {
        expect(feat.title.length, `Empty feature title in ${lp.slug}`).toBeGreaterThan(0);
        expect(feat.description.length, `Empty feature description in ${lp.slug}`).toBeGreaterThan(
          0,
        );
        expect(feat.appSlug.length, `Empty feature appSlug in ${lp.slug}`).toBeGreaterThan(0);
      }
    }
  });
  ...
  it("every landing page has a non-empty brightonMessage", () => {
    for (const lp of LANDING_PAGES) {
      expect(lp.brightonMessage.length, `Empty brightonMessage for ${lp.slug}`).toBeGreaterThan(0);
    }
  });

  it("every landing page has a non-empty ctaLabel and ctaHref", () => {
    for (const lp of LANDING_PAGES) {
      expect(lp.ctaLabel.length, `Empty ctaLabel for ${lp.slug}`).toBeGreaterThan(0);
      expect(lp.ctaHref.length, `Empty ctaHref for ${lp.slug}`).toBeGreaterThan(0);
    }
  });
