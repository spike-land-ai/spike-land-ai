...
export async function getLearnItContent(
  ...
  if (content) {
    // Async increment view count with timeout to prevent connection pool exhaustion
    void Promise.race([
...
export async function createOrUpdateContent(input: CreateContentInput) {
  ...
  const { generatedById: _generatedById, ...inputRest } = input;
  const data = {
    ...inputRest,
    ...(validGeneratedById !== undefined ? { generatedById: validGeneratedById } : {}),
    wikiLinks: links,
  };
...
export async function getContentByTopicSlugs(
  slugs: string[],
  limit = 6,
): Promise<LearnItContent[]> {
