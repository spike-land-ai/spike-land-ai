...
export class FacebookMarketingClient implements IMarketingClient {
  ...
  private async request<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
    ...
    if (fetchError || !response) {
      throw new Error(`Facebook API Error: ${fetchError?.message || "Network error"}`);
    }
    ...
    if (jsonError || data === null || data === undefined) {
      throw new Error(`Facebook API Error: ${jsonError?.message || "Invalid response"}`);
    }
  ...
  private async getAccountCurrency(accountId: string): Promise<string> {
    ...
    const cached =
      this.accountCurrencyCache.get(normalizedId) || this.accountCurrencyCache.get(baseId);
    if (cached) {
      return cached;
    }
    ...
    // Fetch from API
    const { data: response, error } = await tryCatch(
      this.request<FacebookAdAccount>(`/${normalizedId}?fields=currency`),
    );
  ...
  async getCampaignMetrics(
    ...
    const [currency, response] = await Promise.all([
      ...
      this.request<{
        ...
        `/${campaignId}/insights?fields=impressions,clicks,spend,conversions,reach,frequency,ctr,cpc,cpm&time_range=${encodeURIComponent(
          dateRange,
        )}`,
      ),
    ]);
  ...
  private mapCampaign(
    ...
    return {
      ...
      budgetType: hasDailyBudget ? "DAILY" : fb.lifetime_budget ? "LIFETIME" : "UNKNOWN",
