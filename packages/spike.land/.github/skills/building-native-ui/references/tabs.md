...
Always prefer NativeTabs from 'expo-router/unstable-native-tabs' for the best
iOS experience.
...
- The `NativeTabs.Trigger` 'name' must match the route name, including
  parentheses (e.g. `<NativeTabs.Trigger name="(search)">`)
...
Available roles: `search` | `more` | `favorites` | `bookmarks` | `contacts` |
`downloads` | `featured` | `history` | `mostRecent` | `mostViewed` | `recents` |
`topRated`
...
Native tabs don't render headers. Nest Stacks inside each tab for navigation
headers:
...
3. **Trigger name mismatch**: Ensure `name` matches exact route name including
   parentheses
