...
describe("KVLogger", () => {
  ...
  describe("log method", () => {
    ...
    it("should handle log saving errors", async () => {
      ...
      expect(mockConsoleError).toHaveBeenCalledWith("Failed to save log entry:", expect.any(Error));
    });
  });
  ...
  describe("getLogs method", () => {
    ...
    it("should handle log retrieval errors", async () => {
      ...
      expect(mockConsoleError).toHaveBeenCalledWith("Failed to retrieve logs:", expect.any(Error));
      expect(logs).toEqual([]);
    });
  });
});
